...
import { addProcurementCommittee } from "@/lib/helper";
...
export default function AddProcurementCommittee() {
  ...
  const handleAddProcurementCommittee = async (model) => {
    const result = await addProcurementCommittee(model);
    const { isSaved } = result;
    if (isSaved) {
      setSavedNotify(true);
      setTimeout(() => {
        router.push("/admin/committee/procurement");
      }, 500);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (url) {
        ...
        await handleAddProcurementCommittee(model);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    ...
    <div>
      ...
      <Header />
      {savedNotify && (
        <Success message={"Procurement committee has been added successfully"} />
      )}

      <div className={styles.content}>
        <div className={styles.header}>Add a new Procurement Committee</div>
