...
import { addReligiousCommittee } from "@/lib/helper";
...
export default function AddReligiousCommittee() {
  ...
  const handleAddReligiousCommittee = async (model) => {
    const result = await addReligiousCommittee(model);
    const { isSaved } = result;
    if (isSaved) {
      setSavedNotify(true);
      setTimeout(() => {
        router.push("/admin/committee/religious");
      }, 500);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (url) {
        ...
        await handleAddReligiousCommittee(model);
      }
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    ...
    <div>
      ...
      <Header />
      {savedNotify && (
        <Success message={"Religious committee has been added successfully"} />
      )}

      <div className={styles.content}>
        <div className={styles.header}>Add a new Religious Committee</div>
