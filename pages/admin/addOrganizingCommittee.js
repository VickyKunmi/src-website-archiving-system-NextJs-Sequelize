...
import { addOrganizingCommittee } from "@/lib/helper";
...
export default function AddEstateCommittee() {
    ...
    const handleAddOrganizingCommittee = async (model) => {
      const result = await addOrganizingCommittee(model);
      const { isSaved } = result;
      if (isSaved) {
        setSavedNotify(true);
        setTimeout(() => {
          router.push("/admin/committee/organizing");
        }, 500);
      }
    };
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (url) {
          ...
          await handleAddOrganizingCommittee(model);
        }
      } catch (err) {
        console.log(err);
      }
    ...
    return (
      ...
      <div>
        ...
        <Header />
        {savedNotify && (
          <Success message={"Organizing committee has been added successfully"} />
        )}
  
        <div className={styles.content}>
          <div className={styles.header}>Add a new Organizing Committee</div>
