...
import { deleteFinanceCommittee, getFinanceCommittee } from "@/lib/helper";
import { deleteAction } from "@/redux/reducer";
import Link from "next/link";
import { Alert, Button } from "@mui/material";
import { BiCheck, BiUserPlus, BiX } from "react-icons/bi";
import { Table } from "@/components/admin/financeCommitteeTable";
import { getserver } from "@/config";
import { useRequireAuth } from "../auth";

export default function finance({records}) {
  ...
  const deletehandler = async () => {
    if(deleteId) {
      const deletedFinanceCommittee = await deleteFinanceCommittee(deleteId);
      if(deletedFinanceCommittee.isDeleted) {
        router.replace(`/admin/committee/finance`);
      }
  ...
  return (
    ...
    <div className={styles.container}>
      ...
      <div className={styles.item}>
        <Link href="/admin/addFinanceCommittee">
...
export const getServerSideProps = async (ctx) => {
  ...
  const res = await fetch(`${getserver}/api/FinanceCommittee`);
