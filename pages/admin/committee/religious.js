...
import { deleteReligiousCommittee, getReligiousCommittee } from '@/lib/helper';
import { deleteAction } from '@/redux/reducer';
import Link from 'next/link';
import { Alert, Button } from '@mui/material';
import { Table } from '@/components/admin/religiousCommitteeTable';
import { useRequireAuth } from '../auth';


export default function religious({records}) {
  ...
  const deletehandler = async () => {
    if(deleteId) {
      const deletedReligiousCommittee = await deleteReligiousCommittee(deleteId);
      if(deleteReligiousCommittee.isDeleted) {
        router.replace(`/admin/committee/religious`);
      }
  ...
  return (
    ...
    <div className={styles.container}>
      ...
      <div className={styles.item}>
        <Link href="/admin/addReligiousCommittee">
          <Button startIcon={<BiUserPlus/>} className={styles.addExe}>
            Add Religious committee
          </Button>
        </Link>
      </div>
...
export const getServerSideProps = async (ctx) => {
  ...
  const res = await fetch(`${getserver}/api/ReligiousCommittee`);
