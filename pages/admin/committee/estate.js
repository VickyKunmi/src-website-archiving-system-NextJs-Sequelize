...
import { deleteEstateCommittee, getEstateCommittee } from '@/lib/helper';
import { deleteAction } from '@/redux/reducer';
import Link from 'next/link';
import { Alert, Button } from '@mui/material';
import { BiCheck, BiUserPlus, BiX } from 'react-icons/bi';
import { Table } from '@/components/admin/estateCommitteeTable';
import { getserver } from '@/config';
import { useRequireAuth } from '../auth';


export default function estate({records}) {
  ...
  const deletehandler = async () => {
    if(deleteId) {
      const deletedEstateCommittee = await deleteEstateCommittee(deleteId);
      if(deletedEstateCommittee.isDeleted) {
        router.replace(`/admin/committee/estate`);
      }
    ...
    <div className={styles.container}>
      ...
      <div className={styles.wrapper}>
        <div className={styles.item}>
          <Link href="/admin/addEstateCommittee">
            <Button startIcon={<BiUserPlus/>} className={styles.addExe}>
              Add Estate Committee
            </Button>
          </Link>
...
export const getServerSideProps = async (ctx) => {
  ...
  const res = await fetch(`${getserver}/api/EstateCommittee`);
