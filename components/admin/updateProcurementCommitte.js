...
import { updateProcurementCommitteeById, UploadImage } from "@/lib/helper";
import axios from "axios";
import { Button, Input, Paper } from "@mui/material";
import { BiBrush } from "react-icons/bi";
import { Success } from "./Success";

export function UpdateProcurementCommittee({handleClose, data }) {
  ...
  const handleUpdateProcurementCommittee = async (models, id) => {
    const result = await updateProcurementCommitteeById({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (newImg === img) {
        ...
        return await handleUpdateProcurementCommittee(models, id);
      } 
      else {
  ...
  return (
    <form onSubmit={handleUpdate}>
      <Paper elevation={0} sx={{ p: "1em" }}>
        <div>
          {savedNotify && <Success message={"Procurement committee updated successfully"}/>}
        </div>
