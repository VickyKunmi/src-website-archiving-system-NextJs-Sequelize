...
import { UploadImage, updateReligiousCommitteeById } from "@/lib/helper";
import axios from "axios";
import { Button, Input, Paper } from "@mui/material";
import { BiBrush } from "react-icons/bi";
import { Success } from "./Success";

export function UpdateReligiousCommittee({handleClose, data }) {
  ...
  const handleUpdateReligiousCommittee = async (models, id) => {
    const result = await updateReligiousCommitteeById({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (newImg === img) {
        ...
        return await handleUpdateReligiousCommittee(models, id);
      } 
      else {
