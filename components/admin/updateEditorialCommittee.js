...
import { UploadImage, updateEditorialCommitteeById } from "@/lib/helper";
import axios from "axios";
import { Button, Input, Paper } from "@mui/material";
import { BiBrush } from "react-icons/bi";
import { Success } from "./Success";

export function UpdateEditorialCommittee({handleClose, data }) {
  ...
  const handleUpdateEditorialCommittee = async (models, id) => {
    const result = await updateEditorialCommitteeById({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (newImg === img) {
        ...
        return await handleUpdateEditorialCommittee(models, id);
      } else {
  ...
  return (
    <form onSubmit={handleUpdate}>
      <Paper elevation={0} sx={{ p: "1em" }}>
        <div>
          {savedNotify && <Success message={"Editorial committee updated successfully"}/>}
        </div>
