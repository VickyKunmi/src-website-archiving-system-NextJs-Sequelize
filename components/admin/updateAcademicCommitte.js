...
import { UploadImage, updateAcademicCommitteeById } from "@/lib/helper";
import axios from "axios";
import { Button, Input, Paper } from "@mui/material";
import { BiBrush } from "react-icons/bi";
import { Success } from "./Success";

export function UpdateAcademicCommittee({handleClose, data }) {
  ...
  const handleUpdateAcademicCommittee = async (models, id) => {
    const result = await updateAcademicCommitteeById({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (newImg === img) {
        ...
        return await handleUpdateAcademicCommittee(models, id);
      } 
      else {
  ...
  return (
    <form onSubmit={handleUpdate}>
      <Paper elevation={0} sx={{ p: "1em" }}>
        <div>
          {savedNotify && <Success message={"Academic committe updated successfully"}/>}
        </div>
