...
import { UploadImage, updateFinanceCommitteeById } from "@/lib/helper";
import axios from "axios";
import { Button, Input, Paper } from "@mui/material";
import { BiBrush } from "react-icons/bi";
import { Success } from "./Success";

export function UpdateFinanceCommittee({handleClose, data }) {
  ...
  const handleUpdateFinanceCommittee = async (models, id) => {
    const result = await updateFinanceCommitteeById({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (newImg === img) {
        ...
        return await handleUpdateFinanceCommittee(models, id);
      } else {
  ...
  return (
    <form onSubmit={handleUpdate}>
      <Paper elevation={0} sx={{ p: "1em" }}>
        <div>
          {savedNotify && <Success message={"Finance committee updated successfully"}/>}
        </div>
