...
import { UploadImage, updateEstateCommitteeById } from "@/lib/helper";
import axios from "axios";
import { Button, Input, Paper } from "@mui/material";
import { BiBrush } from "react-icons/bi";
import { Success } from "./Success";

export function UpdateEstateCommittee({handleClose, data }) {
  ...
  const handleUpdateEstateCommittee = async (models, id) => {
    const result = await updateEstateCommitteeById({
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      if (newImg === img) {
        ...
        return await handleUpdateEstateCommittee(models, id);
      } 
      else {
